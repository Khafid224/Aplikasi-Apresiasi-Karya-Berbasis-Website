...
if(isset($_POST['submit'])){
   ...
   if(!empty($name)){
      ...
      $success_msg[] = 'nama pengguna berhasil update!';
   }
  ...
  if(!empty($image)){
   if($image_size > 2000000){
      $warning_msg[] = 'Ukuran email terlalu besar!';
   }else{
      ...
      if($verify_old_pass == 1){
         if($c_pass == 1){
            if($empty_new != 1){
               ...
               $warning_msg[] = 'Tolong masukkan password baru!';
            }
         }else{
            $warning_msg[] = 'konfirm password tidak cocok!';
         }
      }else{
         $warning_msg[] = 'password lama tidak cocok!';
      }
  }
...
if(isset($_POST['delete_image'])){
   ...
   if($fetch_old_pic['image'] == ''){
      $warning_msg[] = 'Gambar sudah terhapus!';
   }else{
      ...
      $success_msg[] = 'Gambar berhasil terhapus!';
   }
...
<section class="account-form">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      <h3>update profilmu!</h3>
      <p class="placeholder">namamu</p>
      <input type="text" name="name" maxlength="50" placeholder="<?= $fetch_profile['name']; ?>" class="box">
      <p class="placeholder">emailmu</p>
      <input type="email" name="email" maxlength="50" placeholder="<?= $fetch_profile['email']; ?>" class="box">
      <p class="placeholder">password lama</p>
      <input type="password" name="old_pass" maxlength="50" placeholder="masukkan password lama mu" class="box">
      <p class="placeholder">password baru</p>
      <input type="password" name="new_pass" maxlength="50" placeholder="masukkan password baru mu" class="box">
      <p class="placeholder">konfirmasi password</p>
      <input type="password" name="c_pass" maxlength="50" placeholder="konfirm password baru mu" class="box">
      <?php if($fetch_profile['image'] != ''){ ?>
         <img src="uploaded_files/<?= $fetch_profile['image']; ?>" alt="" class="image">
         <input type="submit" value="hapus gambar" name="delete_image" class="delete-btn" onclick="return confirm('hapus gambar ini?');">
      <?php }; ?>
